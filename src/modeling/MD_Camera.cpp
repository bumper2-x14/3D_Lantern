...
float MD_Camera::getSensevity(){
    return sensevity;
}

void MD_Camera::setSensevity(float _sensevity){
    sensevity = _sensevity ;
}

Mat4f MD_Camera::genLookAt(){
    return Mat4f::lookAt(cameraFront,
...
void MD_Camera::update(bool w,bool s,bool a,bool d,
    ...
    direction.z = std::sin(toRadians(yaw)) * std::cos(toRadians(yaw));

    cameraFront=normalize(direction);
}


void MD_Camera::setView(){
